...
% Extract the full shutter name (file path)
shutterName = xmlStruct.settings.illumination.shutters;
% Convert Windows-style backslashes to Linux-style forward slashes
shutterName = strrep(shutterName, '\', '/');  % Replace \ with /
% Extract the filename from the path (ignoring the path)
[~, shutter, ~] = fileparts(shutterName);  % This will give 'shutters_635_545_series_piezo.xml'

% Remove 'shutters_' and '_series' from the filename
shutter = regexprep(shutter, {'shutters_', '_series'}, {'', ''});  % '635_545_piezo.xml'

% Remove 'confocal_' if it exists (no effect if not present)
shutter = regexprep(shutter, 'confocal_', '');  % '635_545_piezo.xml'

% Remove 'piezo' from the string
shutter = regexprep(shutter, 'piezo', '');  % '635_545.xml'

% Remove the '.xml' extension
shutter = regexprep(shutter, '/.xml$', '');  % '635_545'

% Split the remaining string by '_'
imProps.chnNames = strsplit(shutter, '_');  % This should return {'635', '545'}

% Ensure that only the first two elements (channel names) are considered
imProps.chnNames = imProps.chnNames(1:2);  % Get only the first two elements (channels)

% Calculate the number of channels (should be 2 now)
imProps.nChns = length(imProps.chnNames);

% Set projection property (as per your original code)
...
if pars.justImProps
    ...
    if pars.maxProject && all(projectionExist) && ~pars.overwrite
       ...
       imProps.projection = true;
        ...
        %temp since screwed up
        %dax = dax(:,:,:); %remove this and only use line below
