...
% Get requested FOV
currFolder = fullfile(dataFolder, hybFolders{1});  % Ensure proper file path construction
daxFileStruct = dir(fullfile(currFolder, pars.daxRootDefault));  % Use dir instead of ls
rawDaxFiles = {daxFileStruct.name};  % Extract file names into a cell array
if isinf(pars.fov)
    pars.fov = 1:length(rawDaxFiles);  % Set FOV indices to the number of matching files
end
