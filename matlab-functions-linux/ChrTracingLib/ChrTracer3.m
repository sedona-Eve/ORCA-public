...
%-------------------------- FIT ALL ---------------------------------%
%cross platform compatible (linux not windows prob) Sedona 241219
elseif strcmp(currStepName,'Fit All') 
    saveFolder = CT{handles.id}.saveFolder; % shorthand
    pars = CT{handles.id}.parsFitAll;
    
    % Use the auto-saved versions of crop and fit
    parsCrop = table2struct(readtable(fullfile(saveFolder, 'Pars Crop Spots.csv')));
    parsFit = table2struct(readtable(fullfile(saveFolder, 'Pars Fit Spots.csv')));
    
    % Remove FOVs which lack ROI spots or are not requested
    spotFilesStruct = dir(fullfile(saveFolder, 'fov*_selectSpots.csv'));
    spotFiles = {spotFilesStruct.name}';
    fovsWithSpots = cellfun(@(x) str2double(x(4:6)), spotFiles)'; %#ok<ST2NM>
    
    for f = fovsWithSpots
        disp(['Processing data for FOV ', num2str(f)]);       
        tableSaveName = fullfile(saveFolder, sprintf('fov%03d_AllFits.csv', f));
        ...
        switch answer
            case 'Yes'
                % Crop spots
                spotXY = LoadSpotXY(saveFolder, 'fov', f);             
                ...
                if doMultiFit 
                    saveFits = fullfile(saveFolder, 'fitResults');
